...
pub struct BTreeTable {
    ...
    /// Lazily-built transitive-closure cache for generated-column dependencies.
    /// Populated on first access to `column_graph()`; reset to empty on clone
    /// and on mutation through `columns_mut()`. `pub(crate)` so struct-literal
    /// callers can initialize it to `Default::default()` — external crates
    /// cannot construct the table.
    pub(crate) cached_graph: ResetOnClone<OnceLock<GeneratedColGraph>>,
}
...
impl BTreeTable {
    ...
    pub fn columns_mut(&mut self) -> &mut Vec<Column> {
        self.cached_graph.0 = OnceLock::new();
        &mut self.columns
    }
    ...
    fn column_graph(&self) -> Result<&GeneratedColGraph> {
        if let Some(graph) = self.cached_graph.0.get() {
            return Ok(graph);
        }
        ...
        let _ = self.cached_graph.0.set(graph);
        Ok(self
            .cached_graph
            .0
            .get()
...
mod tests {
    ...
    fn gencol_graph_clone_invalidates_cache() -> Result<()> {
        ...
        assert!(original.cached_graph.0.get().is_some());
        ...
        assert!(cloned.cached_graph.0.get().is_none());
        ...
        assert!(cloned.cached_graph.0.get().is_some());
        Ok(())
    }
    ...
    fn gencol_graph_columns_mut_invalidates_cache() -> Result<()> {
        ...
        assert!(t.cached_graph.0.get().is_some());

        // Any access through columns_mut() wipes the cache, even if we don't mutate.
        let _ = t.columns_mut();
        assert!(t.cached_graph.0.get().is_none());
        Ok(())
    }
}
